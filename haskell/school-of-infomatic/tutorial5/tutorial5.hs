...
eval e (p :&: q)      =  eval e p && eval e q
-- why not            =  not (eval e p && not (eval e q))
...
-- 4.
-- they both true or false under the enviroments
equivalent :: Prop -> Prop -> Bool
equivalent p1 p2 = and [ eval e p1 == eval e p2 | e <- evs ]
  where
    allNames = nub $ names p1 ++ names p2
    evs      = envs allNames
